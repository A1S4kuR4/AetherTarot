...
create table if not exists public.usage_counters (
  ...
  cost_value_usd numeric(12, 6) not null default 0,
  updated_at timestamptz not null default now()
);
...
create table if not exists public.reading_events (
  ...
  phase text check (phase in ('initial', 'final')),
  spread_id text,
  reading_id text,
  initial_reading_id text,
  ...
  completed_initial boolean not null default false,
  completed_final boolean not null default false
);
...
create index if not exists reading_events_created_at_idx
  on public.reading_events (created_at desc);

create index if not exists reading_events_email_created_at_idx
  on public.reading_events (email, created_at desc);

create index if not exists reading_feedback_created_at_idx
  on public.reading_feedback (created_at desc);
...
declare
  ...
  v_email_key text := 'email_daily:' || lower(p_email) || ':' || to_char(v_day_start, 'YYYY-MM-DD');
  v_ip_minute_key text := 'ip_minute:' || p_ip_hash || ':' || to_char(v_minute_start, 'YYYY-MM-DD"T"HH24:MI');
  v_ip_day_key text := 'ip_daily:' || p_ip_hash || ':' || to_char(v_day_start, 'YYYY-MM-DD');
...
begin
  ...
  values
    (v_email_key, 'email_daily', v_day_start, 1, 0, v_now),
    (v_ip_minute_key, 'ip_minute', v_minute_start, 1, 0, v_now),
    (v_ip_day_key, 'ip_daily', v_day_start, 1, 0, v_now)
...
insert into public.beta_testers (email, role, is_active)
values ('643490291@qq.com', 'admin', true)
