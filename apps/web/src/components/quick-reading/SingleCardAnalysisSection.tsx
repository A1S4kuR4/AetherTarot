...
interface SingleCardAnalysisSectionProps {
  readingCard: ReadingCardResult;
  drawnCard: DrawnCard;
}

export function SingleCardAnalysisSection({
  readingCard,
  drawnCard,
        ...
        <div className="flex flex-col gap-6 md:flex-row md:items-start">
          ...
          <div className="min-w-0 flex-1 space-y-4">
            <div className="flex flex-wrap items-center gap-2">
              <span className="chip-warm text-[10px]">{readingCard.position}</span>
              <span className="font-sans text-[11px] font-medium text-text-muted">
                {readingCard.orientation === "reversed" ? "逆位" : "正位"}
              </span>
            </div>
            <div>
              ...
              <p className="text-sm text-text-muted">{readingCard.english_name}</p>
            </div>
            <div className="grid gap-4 md:grid-cols-2">
              <div className="border-l-2 border-paper-border pl-4">
                ...
                <p className="mt-2 font-sans text-sm leading-relaxed text-text-body">
                  {readingCard.name}（{readingCard.orientation === "reversed" ? "逆位" : "正位"}）
                </p>
