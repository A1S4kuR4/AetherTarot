...
const INTENT_CUES = patterns(
  /(?:我|本人)?(?:想|要|需要|准备|计划|打算|意图)|I\s*(?:want|need|plan|intend)|I\s*am\s*(?:going|planning|about)/i,
  /(?:我|本人)?(?:想|要|需要|准备|计划|打算|意图)|i(?:want|need|plan|intend)(?:to)?|iam(?:going|planning|about)to/i,
);

/** Requests or directives: help / show / teach / tell / steps / should / must / do. */
const DIRECTIVE_CUES = patterns(
  /帮我|协助我|教我|示范|告诉我|给我.{0,8}步骤|怎么|如何|你(?:应该|必须|需要|可以)|立即|马上|\b(?:help me|show me|teach me|tell me|give me steps|how can I|how do I|you should|you must|you need to|you can)\b/i,
  /帮我|协助我|教我|示范|告诉我|给我.{0,8}步骤|怎么|如何|你(?:应该|必须|需要|可以)|立即|马上|helpme|showme(?:howto)?|teachme(?:howto)?|tellme(?:howto)?|givemesteps(?:to)?|howcani|howdoi|youshould|youmust|youneedto|youcan/i,
);
...
const SELF_HARM_SAFE_CONTEXTS: SafetyPattern[] = [
  ...patterns(
    /(?:我)?(?:不想|不会|不打算|拒绝|避免)(?:去)?(?:自杀|自残|伤害自己|伤害我自己|结束生命)|我想(?:活着|活下去)/i,
    /(?:i(?:do)?notwantto|idontwantto|iwillnot|idonotplanto|irefuseto)(?:selfharm|suicide|die|killmyself|hurtmyself|endmylife)|iwanttobealive|iwanttolive/i,
  ),
  ...patterns(
...
const VIOLENCE_SAFE_CONTEXTS = patterns(
  /(?:我)?(?:不想|不会|不打算|没有计划|拒绝|避免)(?:去)?(?:杀|伤害|袭击|殴打|弄死)(?:他|她|对方|别人|他们|任何人)/i,
  /i(?:do)?not(?:want|plan|intend)to(?:kill|hurt|attack)(?:him|her|them|someone|anyone)|iwont(?:kill|hurt|attack)(?:him|her|them|someone|anyone)|irefuseto(?:kill|hurt|attack)(?:him|her|them|someone|anyone)/i,
);
...
const HARM_OUTPUT_SAFE_CONTEXTS: SafetyPattern[] = [
  ...
  ...patterns(
    /(?:不要|不能|不应|不应该|不可|拒绝|避免)(?:去)?(?:自杀|自残|伤害自己|伤害他人|杀死自己|杀|袭击|殴打)(?:他|她|对方|别人|他们|任何人)?/i,
    /(?:donot|dont|shouldnot|shouldnt|cannot|cant|youshouldnot)(?:selfharm|suicide|killyourself|hurtyourself|hurt|kill|attack)(?:him|her|them|someone|anyone)?/i,
  ),
];
...
const TREATMENT_SAFE_CONTEXTS = patterns(
  /(?:不要|不能|不应|不应该|不可|不建议|避免|拒绝建议)(?:自行|立即)?(?:停药|停止服药|停止用药|停用药物|停止治疗|拒绝治疗)/i,
  /(?:donot|dont|shouldnot|shouldnt|cannot|cant|youshouldnot)(?:stop(?:taking)?(?:your)?(?:medicine|medication)|stoptreatment|discontinue(?:your)?medication)/i,
);
...
const SELF_HARM_OUTPUT_CORE = patterns(
  /自杀|自残|结束生命|伤害自己|伤害他人|杀死自己|实施暴力/i,
  /selfharm|suicide|killyourself|hurtyourself|hurtsomeone|hurtthem/i,
);

const VIOLENCE_CORE = patterns(
  /(?:杀|杀死|弄死|伤害|袭击|殴打)(?:他|她|对方|别人|他们|任何人)/i,
  /(?:kill|hurt|attack)(?:him|her|them|someone|anyone)|(?:杀|杀死|弄死|伤害|袭击|殴打)(?:他|她|对方|别人|他们|任何人)/i,
);
const IMMEDIATE_VICTIM_DANGER_CORE = patterns(
  /(?:正在|现在|此刻).{0,8}(?:打我|殴打我|伤害我|威胁我|追杀我|有危险)|(?:要|想要|准备).{0,5}(?:杀我|伤害我)|持刀|被困住|无法脱身|生命危险/i,
...
const MANIPULATION_CORE = patterns(
  ...
  /(?:跟踪|监控|监视|报复|操控|勒索|偷窥|试探|控制)(?:他|她|对方|前任|别人|任何人)?|定位(?:他|她|对方|前任)?(?:的)?手机|查(?:他|她|对方|前任)?(?:的)?位置|(?:stalk|track|monitor|control|manipulate|blackmail|retaliate)(?:my|your|the)?(?:ex|partner|someone|anyone|him|her|them|person|people|前任|他|她|对方)/i,
);

const TREATMENT_DISCONTINUATION_CORE = patterns(
  /停药|停止服药|停止用药|停用药物|立即停用药物|停止治疗|拒绝治疗/i,
  /停药|停止服药|停止用药|停用药物|立即停用药物|停止治疗|拒绝治疗|stop(?:taking)?(?:your)?(?:medicine|medication)|stoptreatment|discontinue(?:your)?medication/i,
);

const DIRECT_DIAGNOSIS_CORE = patterns(
  /(?:你|他|她).{0,6}(?:已经|就是|肯定|确定)?(?:患有|得了|确诊为|怀孕)/i,
  /(?:你|他|她)(?:已经|就是|肯定|确定)?(?:患有|得了|确诊为|怀孕)|(?:you|they|he|she)(?:definitely)?(?:have|has)(?:cancer|diabetes|adisease|thecondition)|(?:you|they|he|she)(?:are|is)(?:definitely)?pregnant|(?:you|they|he|she)(?:are|is)diagnosedwith/i,
);
