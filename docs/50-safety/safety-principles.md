...
> 共享 classifier 与生成输出 validator 同样使用“正常分句 + 仅升级的全文风险视图”。风险视图覆盖 `。！？；!?;` 对危险 core/action/target 的逐字符边界插入，但不替代正常分句对局部否定的判断。裸汉字“死”不是当前自伤状态；已验证的中文意图语法允许“想/要/希望/打算/计划/准备”与“死”之间出现一个有限的口语填充词（去、马上、现在、立刻、就），并以句尾、句读/空白或有限语气助词作为正向终结条件，而不是维护复合词后缀黑名单。死神牌、死海、死亡、死党、死心、死磕、死守、作品名和生死观等复合词/领域表达不会仅因包含“死”而 hard-stop。AirTag/GPS 的放置风险要求第三方物品/车辆语境，自有背包等普通用途不因设备名本身 hard-stop。
