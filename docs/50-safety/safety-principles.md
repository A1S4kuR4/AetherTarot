...
> 共享 classifier 与生成输出 validator 同样使用“正常分句 + 仅升级的全文风险视图”。风险视图覆盖 `。！？；!?;` 对危险 core/action/target 的逐字符边界插入，但不替代正常分句对局部否定的判断。裸汉字“死”不是当前自伤状态；已验证的中文意图语法允许“想/要/希望/打算/计划/准备”与“死”之间出现一个有限的口语填充词（去、马上、现在、立刻、就），同时排除死神牌、死海（含“去死海”）、作品名和生死观等领域/专名续接。AirTag/GPS 的放置风险要求第三方物品/车辆语境，自有背包等普通用途不因设备名本身 hard-stop。
