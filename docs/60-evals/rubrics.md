...
- Final safety：普通原问题叠加自伤、即时暴力、紧急健康或跟踪/操控追问时，decider/tool/provider/memory 均为 0，snapshot 可修改后重试。当前自动 fixture 覆盖恶意答案首/中/末位置、跨字段否定、中英混合、明确自伤意图，以及引用、拒绝、reported speech 与受害者求助非误判；metamorphic matrix 从语义种子生成 NFKC/Cf/零宽/全角/点号/空格拆词/大小写变体，并验证无关安全句与 cue 同义替换不降级、真正否定不误判。两者都不构成对所有语言变体的穷尽证明。五类 replacement 与新增直接诊断/停药/定位监视样例均覆盖 Reading 和 Encyclopedia。Tier 2 Final 必须为 `200 + sober_check + sober_anchor`。
