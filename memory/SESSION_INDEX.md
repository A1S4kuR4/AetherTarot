...
- 当前重点：
  ...
  - 最新运行时扩展已把牌库推进到 `42` 张、卡图推进到 `43` 个文件，并在 Encyclopedia 前台显式展示 `42/78` vs `78/78` 覆度
...
- 用途：记录 `2026-04-14` 的回归修复、百科覆度对齐与运行时权杖 / 圣杯扩展
- 当前重点：
  - Playwright 已重新对齐当前 `/new -> /ritual -> /reveal -> /reading -> /journey` 主流程
  - Encyclopedia 前台已显式展示 `42/78 runtime` vs `78/78 knowledge`
  - 运行时牌库已扩展到 `42` 张，卡图已扩展到 `43` 个文件
