...
已完成第一段：API/contract 断言已从 UI smoke flow 抽出为 pps/web/tests/reading-contract.spec.ts，并补充 final 阶段 profile / spread / drawnCards 不一致拒绝用例、follow-up 锚定断言和两阶段语义评测样例文档。后续仍可继续推进更完整的自动化语义评测。
